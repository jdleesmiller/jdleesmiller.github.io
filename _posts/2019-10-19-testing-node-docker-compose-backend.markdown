...
I say 'estimate' here because we can measure 'ground truth' correctness by letting the system loose in the real world and seeing what happens. If we can release quickly, get feedback quickly through great monitoring in production, and the cost of system failure is low, we might not need to estimate. For example, if we deliver pictures of cats at scale, we might just ship to production and measure; if we make antilock braking systems, not so much. In most domains, it is worth investing in testing so we can accurately predict and improve correctness before we go to production.
