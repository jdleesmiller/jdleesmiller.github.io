...
- The integration tests aim to cover all the success and error handling cases in the app's controllers, but they don't exhaustively test all of the possible causes of errors in the model layer, because those are covered in the model tests. For example, the `it('handles a validation error on create', ...)` test checks what happens if the description is missing, but there isn't an integration test for the case where a description that is too long, because there was a model test for that.
   ...
   This effect is a major contributor to the often talked about [test pyramid](https://martinfowler.com/articles/practical-test-pyramid.html), in which we have more model tests than integration tests. In this example, the model layer is too simple for that pattern to emerge. In a large application, however, the test pyramid is a good ideal to strive for, because integration tests generally take longer to run than model tests and usually involve more ceremony (making requests, getting responses and deserializing them, etc.) that makes them more laborious to write. Other things being equal, it's usually best to test at the lowest level you can.
...
We've seen how to write and run model and integration tests for a simple node.js web service with Docker and Docker Compose. Great job!

I hope that shell scripts like `bin/up` will also give you ideas for how to automate common tasks in your Docker Compose projects. We'll see several more of them in subsequent posts.
...
So, why do we test? We test to *estimate correctness*. Good testing provides accurate, cheap estimates of the correctness of a system that let us iterate quickly to improve it.

I say 'estimate' here because we can measure actual 'ground truth' correctness by letting the system loose in the real world and seeing what happens. Sometimes this is exactly what we should do: if we can release and get feedback quickly, have great monitoring and observability in production, and the cost of system failure is low, we may not need to do much testing pre-release. If your system delivers pictures of cats at scale, this could be you; if you make antilock braking systems, it hopefully isn't. In most domains, it is worthwhile to have tests that let us accurately predict and improve correctness before we go to production.
...
Fortunately, there are other ways to decrease test runtimes besides using fakes. A key property of testing is that it is [embarrassingly parallel](https://en.wikipedia.org/wiki/Embarrassingly_parallel). Tests cases are by design independent, so it is straightforward (though not necessarily trivial) to run them in parallel. Some frameworks, [such as rails](https://guides.rubyonrails.org/testing.html#parallel-testing), can do this out of the box. And now that scalable, cloud-based continuous integration systems are commonplace, you can bring a lot of compute power to bear at relatively low cost. Of course, massive parallelism doesn't help so much if you are running the tests on your laptop, but in most cases you don't have to rerun the whole test suite to make progress on a single feature --- usually only a subset of the tests will be relevant. Then you can push the code to the beefy CI boxes to get a final check that the change hasn't broken something in an unexpected part of the application.
